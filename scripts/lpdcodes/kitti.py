...
class KITTILoader(ABC):
    ...
    def _load_gt_image(self, image_path):
        ...
        #print(f"_load_gt_image: {image_path} : {img}")
    ...
    def _get_single_processed_item(self, idx):
        ...
        #print(f"_get_single_processed_item: {image}")
        return image, label

    def _batch_post_processing(self, images, labels):
        """Post processing for a batch."""
        #print(f"nabil: {images}")
        images = np.array(images)
        #print(f"rawkitti: {images}")
        ...
        if self.num_channels == 3:
          ...
          #  print(f"kitti_image : {images}")
            ...
            # subtract imagenet mean
            images -= np.array([[[bb]]])
       # print(f"_batch_post_processing img : {images} label: {labels}")
    ...
    def __next__(self):
        ...
        if self.n < self.n_batches:
            for idx in range(self.n * self.batch_size,
                ...
                images.append(image)
                labels.append(label)
                #print(f"image1 : {images} labels1 : {labels}")
            self.n += 1
            #print(f"image2 : {images} labels2 : {labels}")
